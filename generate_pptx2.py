from pptx import Presentation
from pptx.util import Pt, Emu
from pptx.dml.color import RGBColor
from pptx.enum.text import PP_ALIGN
from pptx.oxml.ns import qn
from lxml import etree
import base64, io

def set_rounded_corners(shape, radius_emu):
    """角丸の四角形にする"""
    spPr = shape._element.spPr
    prstGeom = spPr.find(qn('a:prstGeom'))
    if prstGeom is not None:
        spPr.remove(prstGeom)
    new_prstGeom = etree.SubElement(spPr, qn('a:prstGeom'))
    new_prstGeom.set('prst', 'roundRect')
    avLst = etree.SubElement(new_prstGeom, qn('a:avLst'))
    gd = etree.SubElement(avLst, qn('a:gd'))
    gd.set('name', 'adj')
    gd.set('fmla', f'val {radius_emu}')

prs = Presentation()
prs.slide_width = Emu(7560000)   # 210mm
prs.slide_height = Emu(10692000) # 297mm

slide = prs.slides.add_slide(prs.slide_layouts[6])

W = 7560000
MM = 36000
RADIUS = 8000  # 角丸の半径

# === ヘッダー（フォントはドクター版に合わせる） ===
header = slide.shapes.add_shape(1, 0, 0, W, int(36 * MM))
header.fill.solid()
header.fill.fore_color.rgb = RGBColor(0x0d, 0x94, 0x88)
header.line.fill.background()

tf = header.text_frame
tf.word_wrap = True
p = tf.paragraphs[0]
p.alignment = PP_ALIGN.CENTER
p.space_before = Pt(12)
run = p.add_run()
run.text = "肥満症治療薬の保険適応をチェック"
run.font.size = Pt(40)
run.font.bold = True
run.font.color.rgb = RGBColor(0xFF, 0xFF, 0xFF)

p2 = tf.add_paragraph()
p2.alignment = PP_ALIGN.CENTER
p2.space_before = Pt(4)
run2 = p2.add_run()
run2.text = "ウゴービ® / ゼップバウンド® があなたに使えるか確認してみましょう"
run2.font.size = Pt(20)
run2.font.color.rgb = RGBColor(0xFF, 0xFF, 0xFF)

# === イントロ ===
intro_top = int(40 * MM)
intro = slide.shapes.add_shape(1, int(8 * MM), intro_top, W - int(16 * MM), int(26 * MM))
intro.fill.solid()
intro.fill.fore_color.rgb = RGBColor(0xF0, 0xFD, 0xF4)
intro.line.color.rgb = RGBColor(0xBB, 0xF7, 0xD0)
intro.line.width = Pt(2)
set_rounded_corners(intro, RADIUS)

tf = intro.text_frame
tf.word_wrap = True
p = tf.paragraphs[0]
p.alignment = PP_ALIGN.CENTER
p.space_before = Pt(6)
run = p.add_run()
run.text = "肥満症は「治療できる病気」です。"
run.font.size = Pt(28)
run.font.bold = True
run.font.color.rgb = RGBColor(0x16, 0x65, 0x34)

p2 = tf.add_paragraph()
p2.alignment = PP_ALIGN.CENTER
p2.space_before = Pt(2)
run2 = p2.add_run()
run2.text = "一定の条件を満たす方には、\n肥満症治療薬が保険適用で使用できます。"
run2.font.size = Pt(20)
run2.font.color.rgb = RGBColor(0x16, 0x65, 0x34)

# === セクションタイトル ===
sec_top = int(69 * MM)
sec = slide.shapes.add_textbox(int(8 * MM), sec_top, W - int(16 * MM), int(12 * MM))
tf = sec.text_frame
p = tf.paragraphs[0]
run = p.add_run()
run.text = "保険適応の主な条件"
run.font.size = Pt(24)
run.font.bold = True
run.font.color.rgb = RGBColor(0x0d, 0x94, 0x88)

line = slide.shapes.add_shape(1, int(8 * MM), sec_top + int(11 * MM), W - int(16 * MM), int(1 * MM))
line.fill.solid()
line.fill.fore_color.rgb = RGBColor(0x0d, 0x94, 0x88)
line.line.fill.background()

# === 4つの条件カード ===
conditions = [
    ("1", "BMI 27以上", "身長と体重から計算される\nBMIが27以上であること"),
    ("2", "対象疾患の治療中", "高血圧・脂質異常症・\n2型糖尿病のいずれかで\n薬物治療中"),
    ("3", "BMI別の追加条件", "BMI 35以上：条件1・2を\n満たせば対象\nBMI 27〜35：健康障害が\n2つ以上必要"),
    ("4", "生活習慣改善の実施", "食事・運動療法を行っても\n十分な効果が得られて\nいないこと"),
]

card_w = int(92 * MM)
card_h = int(46 * MM)
gap = int(7 * MM)
margin_x = int(8 * MM)
card_top = int(84 * MM)

for i, (num, title, desc) in enumerate(conditions):
    col = i % 2
    row = i // 2
    x = margin_x + col * (card_w + gap)
    y = card_top + row * (card_h + gap)

    card = slide.shapes.add_shape(1, x, y, card_w, card_h)
    card.fill.solid()
    card.fill.fore_color.rgb = RGBColor(0xF8, 0xFA, 0xFC)
    card.line.color.rgb = RGBColor(0xD1, 0xD5, 0xDB)
    card.line.width = Pt(1)
    set_rounded_corners(card, RADIUS)

    # 番号丸
    circle = slide.shapes.add_shape(9, x + int(3 * MM), y + int(3 * MM), int(9 * MM), int(9 * MM))
    circle.fill.solid()
    circle.fill.fore_color.rgb = RGBColor(0x0d, 0x94, 0x88)
    circle.line.fill.background()
    tf = circle.text_frame
    p = tf.paragraphs[0]
    p.alignment = PP_ALIGN.CENTER
    run = p.add_run()
    run.text = num
    run.font.size = Pt(16)
    run.font.bold = True
    run.font.color.rgb = RGBColor(0xFF, 0xFF, 0xFF)

    # タイトル
    ttl = slide.shapes.add_textbox(x + int(14 * MM), y + int(2 * MM), card_w - int(17 * MM), int(10 * MM))
    tf = ttl.text_frame
    tf.word_wrap = True
    p = tf.paragraphs[0]
    run = p.add_run()
    run.text = title
    run.font.size = Pt(22)
    run.font.bold = True
    run.font.color.rgb = RGBColor(0x1A, 0x1A, 0x1A)

    # 説明
    dtb = slide.shapes.add_textbox(x + int(4 * MM), y + int(14 * MM), card_w - int(8 * MM), card_h - int(16 * MM))
    tf = dtb.text_frame
    tf.word_wrap = True
    p = tf.paragraphs[0]
    run = p.add_run()
    run.text = desc
    run.font.size = Pt(17)
    run.font.color.rgb = RGBColor(0x47, 0x55, 0x69)

# === QRセクション ===
qr_top = card_top + 2 * (card_h + gap) + int(4 * MM)
qr_box = slide.shapes.add_shape(1, int(8 * MM), qr_top, W - int(16 * MM), int(48 * MM))
qr_box.fill.solid()
qr_box.fill.fore_color.rgb = RGBColor(0xF0, 0xF9, 0xFF)
qr_box.line.color.rgb = RGBColor(0x0d, 0x94, 0x88)
qr_box.line.width = Pt(3)
set_rounded_corners(qr_box, RADIUS)

# QR画像
qr_b64 = ""
qr_data = base64.b64decode(qr_b64)
qr_stream = io.BytesIO(qr_data)
slide.shapes.add_picture(qr_stream, int(14 * MM), qr_top + int(4 * MM), int(40 * MM), int(40 * MM))

# QRテキスト
qr_txt = slide.shapes.add_textbox(int(60 * MM), qr_top + int(3 * MM), int(130 * MM), int(42 * MM))
tf = qr_txt.text_frame
tf.word_wrap = True
p = tf.paragraphs[0]
run = p.add_run()
run.text = "スマホで今すぐチェック"
run.font.size = Pt(28)
run.font.bold = True
run.font.color.rgb = RGBColor(0x0d, 0x94, 0x88)

p2 = tf.add_paragraph()
p2.space_before = Pt(4)
run2 = p2.add_run()
run2.text = "QRコードを読み取るだけで\n保険適応の対象か確認できます。\n所要時間はわずか1分です。"
run2.font.size = Pt(18)
run2.font.color.rgb = RGBColor(0x33, 0x41, 0x55)

p3 = tf.add_paragraph()
p3.space_before = Pt(3)
run3 = p3.add_run()
run3.text = "https://obesity-checker-tau.vercel.app"
run3.font.size = Pt(12)
run3.font.color.rgb = RGBColor(0x94, 0xA3, 0xB8)

# === 注意書き ===
note_top = qr_top + int(52 * MM)
note = slide.shapes.add_shape(1, int(8 * MM), note_top, W - int(16 * MM), int(18 * MM))
note.fill.solid()
note.fill.fore_color.rgb = RGBColor(0xFF, 0xFB, 0xEB)
note.line.color.rgb = RGBColor(0xFD, 0xE6, 0x8A)
note.line.width = Pt(1)
set_rounded_corners(note, RADIUS)

tf = note.text_frame
tf.word_wrap = True
p = tf.paragraphs[0]
run = p.add_run()
run.text = "※ このチェックは簡易的なセルフチェックであり、医療行為ではありません。実際の診断・処方は、肥満症治療の教育認定施設にて専門医が行います。チェック結果をもとに、かかりつけ医にご相談ください。"
run.font.size = Pt(11)
run.font.color.rgb = RGBColor(0x92, 0x40, 0x0E)

# === フッター ===
footer_top = note_top + int(20 * MM)
ft = slide.shapes.add_textbox(int(8 * MM), footer_top, W - int(16 * MM), int(8 * MM))
tf = ft.text_frame
p = tf.paragraphs[0]
p.alignment = PP_ALIGN.CENTER
run = p.add_run()
run.text = "日本肥満学会・日本肥満症治療学会「肥満症治療薬の安全・適正使用に関するステートメント」（2025年4月改訂）に基づく"
run.font.size = Pt(9)
run.font.color.rgb = RGBColor(0xB0, 0xB0, 0xB0)

out = "/Users/iwamototatsuya/Desktop/Claude-Projects/obesity-checker/flyer_v2.pptx"
prs.save(out)
print(f"Saved: {out}")
